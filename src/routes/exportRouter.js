...
const hitungTotalJam = (waktuMulai, waktuSelesai) => {
...
const getHariDanTanggal = (tanggalStr) => {
...
router.get('/export-excel/:day', async (req, res) => {
    try {
        ...
        const nikOSH = req.query.nikOSH;
        const nikManager = req.query.nikManager;
        const nikHRD = req.query.nikHRD;
        let ttdOSH = '', namaOSH = '';
        let ttdManager = '', namaManager = '';
        let ttdHRD = '', namaHRD = '', jabatanHRD = '';
        const seenNik = new Set();
        const dataUnik = dataAbsen.filter((item) => {
            if (!item.nik)
                return true;
            // LOGIKA OSH
            if (item.nik === nikOSH) {
                ...
                const isOnlyApproval = item.isApprovalMode === true ||
        ...
        // =========================================================
        dataUnik.slice(1).forEach((item, index) => {
            ...
            try {
                ...
                if (fWs) {
                    ...
                    fWs.eachRow({ includeEmpty: true }, (fRow, rNum) => {
                        ...
                        if (fRow.height)
                            tRow.height = fRow.height;
                        fRow.eachCell({ includeEmpty: true }, (c, cNum) => {
                            const tC = tRow.getCell(cNum);
                            tC.value = c.value;
                            if (c.font)
                                tC.font = JSON.parse(JSON.stringify(c.font));
                            if (c.alignment)
                                tC.alignment = JSON.parse(JSON.stringify(c.alignment));
                            if (c.border)
                                tC.border = JSON.parse(JSON.stringify(c.border));
                            if (c.fill)
                                tC.fill = JSON.parse(JSON.stringify(c.fill));
                            tC.numFmt = c.numFmt;
                        });
                        tRow.commit();
                    });
                    // --- PERBAIKAN MERGE CELLS (ANTI-ERROR TYPESCRIPT) ---
                    const fModel = fWs.model;
                    if (fModel?.merges) {
                        fModel.merges.forEach((m) => {
                            const r = m.split(':');
        ...
        // =========================================================
        const pasangTTDDanNama = (base64Data, nama, colIndex, rowTemplateTTD, rowTemplateNama, jabatan, rowTemplateJabatan // <-- Parameter opsional ditaruh paling belakang
        ) => {
